...
class BeautifulPicture():
    ...
    def __init__(self):  #类的初始化操作
        ...
        self.web_urls = [
            ...
            'https://characterdesignreferences.com/art-of-animation-7/art-of-my-neighbor-totoro?rq=totoro',
            'https://characterdesignreferences.com/art-of-animation-2/kikis-delivery-service?rq=kiki',
            'https://characterdesignreferences.com/art-of-animation-9/art-of-coco'
                         ]  #要访问的网页地址
        self.folder_path = 'C:/Users/Administrator/Desktop/pic/'  #设置图片要存放的文件目录

    def get_pic(self):
        print('开始网页get请求')
        for web_url in self.web_urls:
            targetFolderPath = self.folder_path+'/'+web_url.split("/").pop().split("?")[0]

            r = self.request(web_url)
            print('开始获取所有a标签')
            all_a = BeautifulSoup(r.text, 'lxml').find_all('img', class_="thumb-image")  #获取网页中的class为cV68d的所有a标签
            print('开始创建文件夹')
            self.mkdir(targetFolderPath)  #创建文件夹
            print('开始切换文件夹')
            os.chdir(targetFolderPath)   #切换路径至上面创建的文件夹

            for item in all_a: #循环每个标签，获取标签中图片的url并且进行网络请求，最后保存图片
                img_name = item["alt"].split('.jpg')[0]
                img_url_final = item["data-src"]
